...
data = [
    {
        "messages": [
            ...
            {
                "role": "assistant",
                "content": "我是{}的小助手，请问您有什么需要帮忙的嘛？".format(name)
            }
        ]
    }
]
...
请你介绍一下你自己<|im_end|>
<|im_start|>assistant
我是高考志愿咨询师琪琪的小助手，请问您有什么需要帮忙的嘛？<|im_end|>
...
通过两者的对比我们其实就可以很清楚的看到，在300轮的时候模型已经学会了在我问 “你是谁” 或者说 “请你介绍一下我自己” 的时候回答 “我是高考志愿咨询师琪琪的小助手，请问您有什么需要帮忙的嘛？”。

但是两者的不同是在询问 “你是我的小助手” 的这个问题上，300轮的时候是回答正确的，回答了 “是” ，但是在600轮的时候回答的还是 “我是高考志愿咨询师琪琪的小助手，请问您有什么需要帮忙的嘛？” 这一段话。这表明模型在第一批次第600轮的时候已经出现严重的过拟合（即模型丢失了基础的能力，只会成为某一句话的复读机）现象了，到后面的话无论我们再问什么，得到的结果也就只能是回答这一句话了，模型已经不会再说别的话了。因此假如以通用能力的角度选择最合适的权重文件的话我们可能会选择前面的权重文件进行后续的模型转化及整合工作。
...
2. **增加常规的对话数据集从而稀释原本数据的占比**：这个方法其实就是希望我们正常用对话数据集做指令微调的同时还加上一部分的数据集来让模型既能够学到正常对话，但是在遇到特定问题时进行特殊化处理。比如说我在一万条正常的对话数据里混入两千条和小助手相关的数据集，这样模型同样可以在不丢失对话能力的前提下学到小助手这句话。这种其实是比较常见的处理方式，大家可以自己动手尝试实践一下。
...
Log: Exit!
```
可以看到模型已经严重过拟合，回复的话就只有 “我是高考志愿咨询师琪琪的小助手，请问您有什么需要帮忙的嘛？” 这句话。我们下面可以通过对比原模型的能力来看看差异。
...
### 参数更改（防止过拟合参数修正）
本文作者修改了一些参数，参考以下参数可以解决过拟合现象。
1. 将数据集的重复次数降低
   ![降低重复次数](images/降低重复次数.png)
   执行如下代码，生成对应数据集
...
2. 修改微调配置文件
   ...
   配置文件参数可参考官网文档：https://github.com/open-mmlab/mmengine/blob/main/docs/zh_cn/tutorials/param_scheduler.md

   修改`internlm2_1_8b_qlora_alpaca_e3_copy.py`
    ```diff
    # 修改max_length
    - max_length = 1024
    + max_length = 256
    ...
    ```
3. 修改后测试结果
   ![参数更改后测试结果](images/参数更改后测试结果.png)
...
```
使用adapter方式的效果如下图
![adapter方式的效果图](images/adapter方式的效果图.png)
...
我们需要输入以下命令运行 `/root/personal_assistant/code/InternLM` 目录下的 `web_demo.py` 文件。
...
## 开发机准备
使用 `Cuda11.7-conda` 镜像，然后在资源配置中，使用 `30% A100 * 1` 的资源。
...
![微调前](images/before_finetune.png)
