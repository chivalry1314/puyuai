...
![rag_vs](images/rag_vs.png)

## 环境配置

镜像 使用 `Cuda11.7-conda` 镜像，然后在资源配置中，使用 `30% A100 * 1` 的选项，然后立即创建开发机器。
...
茴香豆工具在 `Intern Studio` 开发机的安装工作结束。如果部署在自己的服务器上，参考上节课模型下载内容或本节部分内容下载模型文件。
...
![rag_answer1](images/rag_answer1.png)
![rag_answer2](images/rag_answer2.png)

`InternLM2-Chat-7B` 的关于 `huixiangdou` 问题的原始输出：
![no_rag_answer](images/no_rag_answer.png)
...
![remote_config](images/remote_config.png)
...
3. 针对远程服务器，如我们的 `Intern Studio` 开发机，我们需要设置端口映射，转发端口到本地浏览器。

4. 在本地浏览器中输入 127.0.0.1:7860 进入 Gradio 对话 Demo 界面，开始对话。

![huixiangdou_gradio](images/huixiangdou_gradio.png)

![huixiangdou_gradio2](images/huixiangdou_gradio2.png)
